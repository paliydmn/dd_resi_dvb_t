...
ADAPTER_CONF_FILE = Path("app/config/adapters_config.json")
...
def save_adapters_to_file():
    ...
    with ADAPTER_CONF_FILE.open('w') as file:
...
def load_adapters_from_file():
    ...
    if ADAPTER_CONF_FILE.exists():
        try:
            with ADAPTER_CONF_FILE.open('r') as file:
